...
- `tools/convert_stage_json_landscape.mjs`：TOOL-01 横版 CLI 转译器，把 portrait authoring stage JSON 可复现地转成 landscape derivative JSON；默认用于 review/export，也可通过 `--id story_###` 生成正式 runtime id。CLI、Browser Builder 与 custom pipeline 技术说明统一见 `docs/tech/landscape_stage_json_toolchain_tech.md`。
...
- 如果需要静态横版 JSON，先按 `docs/tech/landscape_stage_json_toolchain_tech.md` 走“编辑器导出 -> 规范化 -> 横版转译 -> 正式 runtime stage JSON -> StageLoader 加载”的流水线，不要手工复制改写关卡数据。
