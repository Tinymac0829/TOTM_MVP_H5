...
#### 移动速度规则
- 移动速度：5.0 格/秒（源自逆向报告 `GameplayConstants._playerSpeed`）
- 移动时间 = 移动距离 / 移动速度
- 例如：移动 3 格需要 0.6 秒
...
| 参数 | 值 | 单位 | 置信度 | 来源 |
|------|-----|------|--------|------|
| 玩家移动速度 | 5.0 | 格/秒 | 🟢 | GameplayConstants._playerSpeed |
...
- 移动距离：N 格
- 移动时间：N / 5.0 秒
- 每个固定步长（0.02s）的进度增量：0.02 / (N / 5.0) = 0.1 / N

**示例**：
- 移动 1 格：moveProgress += 0.1，需要 10 个固定步长（0.2s）
- 移动 2 格：moveProgress += 0.05，需要 20 个固定步长（0.4s）
- 移动 5 格：moveProgress += 0.02，需要 50 个固定步长（1.0s）
...
| 收集物 | 向收集物方向滑动 | 角色经过时收集物消失，角色继续移动 |
...
| 移动速度不合适 | 手感太快/太慢 | 调整为 4.0 或 6.0（需重新验证） |
