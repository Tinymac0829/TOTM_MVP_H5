...
| ENG-04 | IN_PROGRESS | `R-007,R-008,R-009,R-011` | 实现核心移动手感 | `R-009` 三层坐标域运动与 `R-008` `0.1s/100ms` 输入缓冲的当前代码批次已完成。`CoordinateSystem`、`PlayerController`、`Renderer`、`main.js` 与模块缓存 query 已通过自动校验和加载 `eng04_input_buffer_v1` 的真实浏览器回归；最终任务关闭仍等待明确收口指令 |
| ENG-05 | IN_PROGRESS | `R-012,R-013,R-015` | 实现最小玩法 UI | HUD/状态流已通过 `2026-05-01` 联合真实浏览器回归；该回归覆盖 ENG-04 world-units 运动、Renderer 坐标适配、短距离过冲处理和 `100ms` 输入缓冲。按钮、弹窗、HUD 同步、输入锁定和点击缩放时机均复测为 `PASS` |
| LVL-01 | TODO | `R-010,R-011,R-012,R-017` | 接入 `Story 1` | 关卡可完整游玩并进入失败/通关 |
| QA-01 | TODO | `R-006,R-007,R-008,R-009,R-017` | 完成 `Story 1` 手感验证 | 基线设备上输入到响应延迟低于 `50ms`、四向识别测试正确、且 `100ms` 缓冲窗口内输入可生效 |
...
| `2026-05-02` 到 `2026-05-03` | `v0.2.0` | `Story 2` 接入（LVL-02）、回归检查（QA-02） |
