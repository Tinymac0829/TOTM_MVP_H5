...
| QA-01 | TODO | `R-006,R-007,R-008,R-009,R-017` | 完成 `Story 1` 手感验证 | 基线设备上输入到响应延迟低于 `50ms`、四向识别测试正确、且 `100ms` 缓冲窗口内输入可生效；本次文档收口不直接关闭该独立 QA 项 |
...
| `v0.1.1` | IN_PROGRESS | `ENG-04, ENG-05, LVL-01, QA-01, OPS-01` | 核心手感 + `Story 1` + URL 访问 | ENG-04、ENG-05 与 LVL-01 已基于 `2026-05-01` 真实浏览器回归收口；QA-01 与 OPS-01 仍需单独关闭 |
...
| `2026-05-01` | `STATUS` | 基于已归档的 ENG-04 × ENG-05 真实浏览器联合回归结果，正式将 ENG-04、ENG-05 与 LVL-01 收口为 `DONE`。 | `story_001` 主链路进入主分支基线；QA-01 因仍缺少独立的 `50ms` 基线设备量化收口记录，继续保持 `TODO`。 |
