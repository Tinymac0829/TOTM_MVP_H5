...
| ENG-04 | DONE | `R-007,R-008,R-009,R-011` | 实现核心移动手感 | 滑到受阻才停、撞墙停下、当前支持瓦片碰撞与输入缓冲手感正确，并已完成浏览器内人工手感验收 |
| ENG-05 | DONE | `R-012,R-013,R-015` | 实现最小玩法 UI | 开始、失败、通关、重开、重复游玩流程可见且可用；已结合 `ENG-04` 完成真实浏览器联合验收 |
...
| `2026-04-26` | `VALIDATION` | `ENG-04 × ENG-05` 已完成真实浏览器联合验收：`story_001` 的启动、滑行、收集/HUD、通关、重复游玩与点击缩放均通过；`eng04_death_validation` 本地验证关卡中的 Spikes 死亡、失败弹窗、重开恢复、输入屏蔽与点击缩放均通过。 | `ENG-04` 与 `ENG-05` 均可标记为 `DONE`；收集物提前消失问题已修复并回归通过，后续浏览器回归需强制刷新或无痕窗口避免 ES Module 缓存误判。 |
