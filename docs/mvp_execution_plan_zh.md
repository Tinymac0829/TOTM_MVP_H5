...
- `FROZEN` 输入缓冲：`0.02s`
- `FROZEN` 玩家实际移动速度：`8.0 tiles/s`（`_runSpeed 5.0 × _gameStageScale 1.6`）
- `FROZEN` 四向滑行直到受阻才停
...
| ENG-04 | IN_PROGRESS | `R-007,R-008,R-009,R-011` | 实现核心移动手感 | 核心链路已通过首轮浏览器验收；`R-009` 已按逆向新结论校正为实际 `8.0 tiles/s`，代码回改与浏览器回归待执行 |
| ENG-05 | IN_PROGRESS | `R-012,R-013,R-015` | 实现最小玩法 UI | HUD/状态流已通过首轮联合验收；因 `ENG-04` 移动速度基线需从实际 `5.0` 回改到 `8.0 tiles/s`，需随 ENG-04 重新联调按钮、弹窗和输入锁定时机 |
...
| `2026-04-27` | `BASELINE` | 根据新版逆向报告校正 `R-009`：`_runSpeed = 5.0` 是 base 值，需乘以 `_gameStageScale = 1.6`，实际移动速度为 `8.0 tiles/s`。 | 前一轮 `ENG-04/ENG-05` 浏览器验收是在实际 `5.0 tiles/s` 实现上完成，需先回改代码再重新执行移动手感、收集/HUD、通关、死亡、弹窗输入锁定与点击缩放回归；在回归完成前 `ENG-04/ENG-05` 状态退回 `IN_PROGRESS`。 |
