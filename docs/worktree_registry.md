...
| `2026-05-01` | `STATUS` | Formally marked `ENG-04` and `ENG-05` as `DONE` based on the archived joint real-browser regression results. | The task registry now matches the master-branch baseline; `LVL-01` is closed in the execution plan, while `QA-01` remains a separate closure item. |
