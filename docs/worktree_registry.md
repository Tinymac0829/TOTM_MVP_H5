...
| `ENG-04` | `codex/eng-04` | `WORKTREE_ROOT/ENG-04` | `TBD` | `IN_PROGRESS` | `2026-04-27` | `R-009` has been corrected from the updated reverse findings: `_runSpeed = 5.0` is the base value and effective movement should be `_runSpeed × _gameStageScale = 8.0 tiles/s`; code correction and real-browser regression are pending, so the task is no longer considered DONE. |
| `ENG-05` | `codex/eng-05` | `WORKTREE_ROOT/ENG-05` | `TBD` | `IN_PROGRESS` | `2026-04-27` | The minimal gameplay UI passed the first joint validation pass, but must be regressed after `ENG-04` effective speed changes from `5.0` to `8.0 tiles/s`, especially fail/clear popups, input lockout, button availability timing, and HUD sync. |
...
| `2026-04-27` | `BASELINE` | The updated reverse report confirms effective movement should be `8.0 tiles/s`; the prior implementation treated `_runSpeed = 5.0` base speed as effective speed. | `ENG-04/ENG-05` return to `IN_PROGRESS`; next steps are document baseline sync, code correction, and a new real-browser manual validation pass. |
