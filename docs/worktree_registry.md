...
| `2026-04-26` | `VALIDATION` | `ENG-04 × ENG-05` completed real-browser joint validation in `WORKTREE_ROOT/ENG-04`, and both `ENG-04` and `ENG-05` were marked `DONE` at that time. | This is now a historical record superseded by the `2026-04-27` runspeed baseline reopen. |
| `2026-04-27` | `BASELINE` | The then-updated reverse report confirmed effective movement should be `8.0 tiles/s`; the prior implementation treated `_runSpeed = 5.0` base speed as effective speed. | This conclusion is superseded by the `2026-04-28` record; retained as history only. |
| `2026-04-28` | `BASELINE` | Reverse report and manual frame-by-frame validation confirm `_runSpeed = 5.0 world units/s`, `TileSize = 0.12 world units/tile`, and about `41.67 tiles/s` in the current tile-grid code; `_gameStageScale = 1.6` does not participate in player tile displacement conversion. | This baseline remains authoritative and is now implemented and regressed by the `2026-05-01` ENG-04 code/validation records. |
| `2026-04-29` | `DESIGN` | ENG-04 now uses a stricter three-coordinate-domain plan and requires a coordinate module, `PlayerController` world-coordinate fields, fixed-step overrun handling inside `PlayerController`, and `Renderer` adaptation to `worldX/worldZ`. | The design direction is now represented by the `2026-05-01` implementation and real-browser regression records; remaining status is task closeout, not code correction. |
| `2026-04-30` | `BASELINE` | ENG-04 corrected `R-008` input buffering to `0.1s/100ms`, with buffer countdown in `update(dt)` and player displacement still in `fixedUpdate`; spatial positioning uses option C, adding center APIs without switching current origin coordinate semantics. | This baseline is now implemented and revalidated by the `2026-05-01` code and real-browser regression records. |
| `2026-05-01` | `CODE` | Completed the ENG-04 code pass for the current baseline: added `CoordinateSystem` half-tile and center APIs while preserving origin semantics, moved input-buffer countdown to `PlayerController.update(deltaTime)`, kept displacement in `fixedUpdate`, and updated module cache queries to `eng04_input_buffer_v1`. | Automated syntax and behavior checks passed for movement, collection, death, clear, buffer expiry/overwrite, and chained-turn semantics. |
...
| `2026-06-05` | `VALIDATION` | LVL-02 and QA-02 are closed on the master branch by Android-device validation through the GitHub Pages public URL. Story 1 startup, Story 1 -> Story 2 transition, `?stage=story_002` direct entry, Story 2 collection counts, Spikes death/fail restart, and Story 2 clear fallback to Story 1 all passed. | `LVL-02`, `QA-02`, and `v0.2.0` are now marked `DONE`; no dedicated QA-02 worktree was created. |
| `2026-06-08` | `STATUS` | Story 3 integration and the stage-editor naming documentation were synced on the master branch through `a21b7f8` and `81c73f5`. `story_003` is loadable, Story 3 validation is recorded, and editor info/export/review display names now derive from ids with leading-zero trimming. | `LVL-03` is now marked `DONE`; `v0.3.0` remains open until the formal QA-03 Story 1-3 regression pass is closed. No dedicated LVL-03 worktree was created. |
| `2026-06-11` | `DOC` | Added `docs/tech/format_stage_json_tech.md` and completed the formatter validation pass for `tools/format_stage_json.mjs`, including usage, formatting, warning, and output-shape checks. | `format_stage_json` now has a tracked technical note and a recorded validation pass on the master branch. No dedicated worktree was created. |
| `2026-06-14` | `QA` | Closed QA-03 on the master branch with local browser automation and supporting stage-data validation. Default Story 1 startup, Story 1-3 direct entries, Story 1 -> Story 2 -> Story 3 -> Story 1 progression, Story 2/3 Spikes fail restart, and collection/event flow all passed. | `QA-03` and `v0.3.0` are now marked `DONE`. This pass did not rerun Android-device or GitHub Pages manual validation. No dedicated QA-03 worktree was created. |
