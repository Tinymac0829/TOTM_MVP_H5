...
| `ENG-04` | `codex/eng-04` | `WORKTREE_ROOT/ENG-04` | `TBD` | `IN_PROGRESS` | `2026-05-01` | `R-009` three-coordinate-domain implementation and `R-008` `0.1s/100ms` input buffer code pass are complete in the ENG-04 worktree. `CoordinateSystem`, `PlayerController`, `Renderer`, and module cache queries have passed automated checks and real-browser regression with `eng04_input_buffer_v1`; task closure remains pending explicit closeout. |
| `ENG-05` | `codex/eng-05` | `WORKTREE_ROOT/ENG-05` | `TBD` | `IN_PROGRESS` | `2026-05-01` | The minimal gameplay UI and ENG-04 integration path have passed the `2026-05-01` real-browser regression after world-units movement, Renderer coordinate adaptation, short-move overrun handling, and the corrected `100ms` input buffer. Fail/clear popups, input lockout, button availability timing, HUD sync, and click/resize behavior were all rechecked as `PASS`. |
