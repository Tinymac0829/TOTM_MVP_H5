...
- `E:\GameDev\H5\Project\TOTM_MVP\TOTM_MVP_Dev` 保留给宏观讨论、版本管理与总进度同步使用。
- 具体任务实现统一放在 `E:\GameDev\H5\Project\TOTM_MVP\TOTM_MVP_Dev_worktrees` 下的 sibling worktree 中进行。
- 默认分支命名规则：`codex/<task-id-lower>`。
- 默认 worktree 路径规则：`E:\GameDev\H5\Project\TOTM_MVP\TOTM_MVP_Dev_worktrees\<TASK-ID>`。
...
| Path | 角色 |
|---|---|
| `E:\GameDev\H5\Project\TOTM_MVP\TOTM_MVP_Dev` | 主工作区：宏观讨论、规划文档、版本基线、状态协调 |
| `E:\GameDev\H5\Project\TOTM_MVP\TOTM_MVP_Dev_worktrees` | 隔离任务执行 worktree 的容器根目录 |

## 当前 Worktree

| Task ID | Branch | Worktree Path | Session ID | Status | Last Update | Notes |
|---|---|---|---|---|---|---|
| `PM-02` | `codex/pm-02` | `E:\GameDev\H5\Project\TOTM_MVP\TOTM_MVP_Dev_worktrees\PM-02` | `019d62cd-a740-79c3-b420-88f6f37d2128` | `DONE` | `2026-04-21` | 技术设计文档任务已全部完成，产出文档：PM-02 核心运行时、ENG-02 关卡格式、ENG-03 输入层、ENG-04 移动手感、ENG-05 HUD 状态流、调试面板方案。 |

## 更新日志

| Date | Type | Summary | Impact |
|---|---|---|---|
| `2026-04-06` | `INIT` | 建立 sibling worktree 容器策略，并创建 `PM-02` 的首个任务 worktree。 | 并行开发现在可以在隔离分支和集中台账下启动。 |
| `2026-04-06` | `SAFE_GIT` | 已将 `E:/GameDev/H5/Project/TOTM_MVP/TOTM_MVP_Dev_worktrees/PM-02` 登记到全局 Git `safe.directory`。 | 专属任务 thread 现在可以在该外部 worktree 内直接运行 Git，而不会触发所有权报错。 |
